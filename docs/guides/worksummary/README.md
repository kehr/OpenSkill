# Work Summary Skill User Guide

Work Summary automation for managers. Fetches team weekly report emails from Outlook, summarizes and aggregates with risk analysis, and generates your personal report in your writing style.

## Prerequisites

- macOS with Microsoft Outlook installed and running
- Python 3.9+
- Claude Code or JoyCode with the `worksummary` skill installed

## Quick Start

```
/worksummary configure
```
...
```
/worksummary summarize this week
/worksummary summarize last week
/worksummary summarize 2026-03-23 to 2026-03-29
```

Fetches weekly report emails from Outlook, filters by your configured sender list and keyword, then generates a per-person summary. Output is written to `worksummary/output/YYYY-WNN/individual-summaries.md`.

### Aggregate Team Report

```
/worksummary aggregate
```
...
Output: `worksummary/output/YYYY-WNN/team-aggregate.md`

### Generate Your Report

```
/worksummary generate my report
```

Creates your personal weekly report based on the team aggregate, written in your style. Requires running `learn-style` first.

Output: `worksummary/output/YYYY-WNN/my-report.md`

### Learn Your Writing Style

```
/worksummary learn my style
```
...
- **Output directory**: where reports are saved (default: `worksummary/output/`)
...
The skill ships with default settings in `skills/worksummary/config/`. You can override any setting by creating files in `.openskill/worksummary/config/`.
...
Customize how the AI analyzes risks and priorities. Copy the built-in `skills/worksummary/config/rules.md` to `.openskill/worksummary/config/rules.md` and edit:
...
### Runtime Data (.openskill/worksummary/)

```
.openskill/worksummary/
  config/
...
This directory is created automatically on first run. It can live at project level (`.openskill/worksummary/`) or user level (`~/.openskill/worksummary/`).

### Output (worksummary/output/)
...
Default location is `worksummary/output/` in the current working directory. Override with `output_dir` in config.json.

## Workflow

Typical weekly workflow:

1. **Monday/Friday**: `/worksummary summarize this week` -- fetch and summarize team reports
2. **Review**: check individual summaries, correct any issues
3. **Aggregate**: `/worksummary aggregate` -- generate team overview with risk analysis
4. **Generate**: `/worksummary generate my report` -- create your personal report
5. **Review and send**: review the draft, make edits, then use it

### First-Time Setup

1. `/worksummary configure` -- set up sender list and preferences
2. `/worksummary learn my style` -- provide 2-5 past report samples
...
- You can always override the date range in your message: `/worksummary summarize 2026-03-10 to 2026-03-16`
